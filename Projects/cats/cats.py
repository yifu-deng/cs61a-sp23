    ...
    for i in range(m + 1):
        for j in range(n + 1):
            if i == 0:
                ...
                dp[i][j] = i
            elif typed[i - 1] == source[j - 1]:
                dp[i][j] = dp[i - 1][j - 1]
            else:
                dp[i][j] = 1 + min(dp[i][j - 1], dp[i - 1][j], dp[i - 1][j - 1])
    return dp[m][n] if dp[m][n] <= limit else -1
...
def fastest_words(match):
    ...
    for i in word_indices:
        fastest_player = 0
        fastest_time = get_all_times(match)[0][i]
        for j in player_indices:
            if get_all_times(match)[j][i] < fastest_time:
                fastest_time = get_all_times(match)[j][i]
                fastest_player = j
        fastest[fastest_player].append(get_all_words(match)[i])
    return fastest
    # END PROBLEM 10
