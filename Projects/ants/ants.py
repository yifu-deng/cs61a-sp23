...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not curr_place.is_hive:
            if curr_place.bees and self.lower_bound <= count <= self.upper_bound:
    ...
    def throw_at(self, target):
        ...
        if target:
            ...
            def slow_action(gamestate):
                nonlocal target_slow_turns
                if target_slow_turns > 0:
                    target_slow_turns -= 1
                    if target_slow_turns > 0 and gamestate.time % 2 == 0:
...
class LaserAnt(ThrowerAnt):
        ...
        while place:
            if not place.is_hive:
                for bee in place.bees:
                    dis[bee] = distance
                if place.ant:
                    dis[place.ant] = distance
                    if place.ant.is_container and place.ant.ant_contained:
                        dis[place.ant.ant_contained] = distance
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.damage - self.insects_shot * 0.0625 - 0.25 * distance
